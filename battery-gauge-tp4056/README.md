...
|---|---|
| Arduino UNO | |
| TP4056 모듈 (USB-C) | `B+/B-/OUT+/OUT-` 4패드 = 보호회로 있는 버전 |
| 18650 셀 | 보호회로 내장 권장 |
...
18650 만충 4.2V는 5V ADC 범위 안이라 직결도 가능하다. 하지만 아두이노가 꺼진 상태에서
A0에 4.2V가 걸리면 내부 보호 다이오드를 통해 역전류가 흐른다. 반으로 나누면 그 위험이 없다.

저항 두 개를 **직렬로 잇고 그 사이에서 선을 뽑아** A0로 보낸다. 같은 값이면 정확히 절반이 된다.
...
그래도 **어디까지나 추정이다.** 휴대폰 배터리 %가 부정확한 것도 같은 이유다. 정확한 잔량이 필요하면 전압이 아니라 **전류를 적산**해야 하고, 그건 쿨롱 카운터가 들어간 fuel gauge IC(MAX17043, MAX17048 등, I2C)의 몫이다.
...
### TP4056 충전 완료가 4.2V를 뜻하지 않았다

이 검증에서 가정 하나가 실측에 뒤집혔다.

파란 LED(STDBY, 충전 완료)가 켜졌으므로 배터리가 4.2V일 것으로 보고, **그걸 기준 삼아
